...
regions_list <- lapply(get_demo_regions(), rtracklayer::import, format="bed")
regions_grl <- GRangesList(regions_list)
names(regions_grl) <-  basename(get_demo_regions())
...
metadata. This is done by using the region_metadata parameters, which must
...
they can be grouped using the group_regions_by parameter, which must be a
...
attribute of the `GRanges` object, or the first line of each individual mcols
...
# Analysis steps and further parameters

A full metagene analysis consists of seven steps. Calling the `new` and 
...
possible to carry these out one by one. Carrying the steps separately
can also make the `metagene`work flow easier to understand, and put
the many available parameters into context. The seven steps are:

* `metagene$new` extracts the coverages of every BAM files over the 
  selected regions.
* `group_coverages` normalizes the coverages, then groups them across 
  design groups.
* `bin_coverages` bins the coverages across the selected regions.
* `split_coverages_by_regions` uses region metadata to split the
  binned coverages into groups.
* `calculate_ci` calculates the mean coverages of all region groups
  across all design groups, as well as their confidence intervals.
* `add_metadata` joins region and design metadata to the confidence 
  intervals.
